        ...
        o.rows.forEach(function (val, i) {
            ...
            o.columns.forEach(function (v, i) {
                if (val[v.data]) {
                    ...
                    //add cell
                    if (val.rowGroupHeader && i < 2) {
                        //only make the first 2 cells of a rowGroupHeader bold
                        child = child.ele("ss:Cell").att("ss:StyleID", "1");
                    } else {
                        child = child.ele("ss:Cell");
                    }
                    if (typeof val[v.data] !== 'function') {
                        if (typeof val[v.data] === 'object') {
                            if (val[v.data] instanceof Date) {
                                child = child.ele("ss:Data").att("ss:Type", "DateTime").raw(_isoDateString(new Date(val[v.data]))).up().up();
                            } else {
                            ...
                            if ((typeof val[v.data]) === 'boolean') {
                                ...
                                child = child.ele("ss:Data").att("ss:Type", "Number").txt(val[v.data]).up().up();
                            } else {
                                var str = val[v.data].split('\u000b').join(' ');
                                child = child.ele("ss:Data").att("ss:Type", "String").txt(str).up().up();
                            }
                        }
                    }
                } else {
